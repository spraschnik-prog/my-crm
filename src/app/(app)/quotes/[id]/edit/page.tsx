...
export default function EditQuotePage() {
  ...
  const [form, setForm] = useState({
    client_id: '', issue_date: '', expiry_date: '', tax_rate: '0', discount: '0', notes: '',
  })
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-8">
        <Link href={`/quotes/${id}`} className="text-slate-400 hover:text-slate-700"><ArrowLeft className="w-5 h-5" /></Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="w-64 space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <input type="number" value={form.discount} onChange={e => setForm(f => ({ ...f, discount: e.target.value }))} min="0" step="0.01"
                  className="flex-1 border border-slate-300 rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" />
              </div>
              <div className="flex items-center gap-2">
        ...
        <div className="flex gap-3">
          ...
          <Link href={`/quotes/${id}`} className="text-sm font-medium text-slate-500 px-4 py-2 rounded-lg border border-slate-200">Cancel</Link>
        </div>
      </form>
    </div>
  )
