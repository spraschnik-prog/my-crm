...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    supabase.from('products').select('*').eq('id', id).single().then(({ data }) => {
      if (data) setForm({ name: data.name, description: data.description ?? '', price: String(data.price), unit: data.unit })
    })
  }, [id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('products').update({
      name: form.name, description: form.description || null, price: parseFloat(form.price) || 0, unit: form.unit,
    }).eq('id', id)
