...
export default function EditInvoicePage() {
  ...
  const [form, setForm] = useState({
    client_id: '', issue_date: '', due_date: '', tax_rate: '0', discount: '0', notes: '',
  })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (inv) {
        ...
        setItems((inv.invoice_items ?? []).map((i: any) => ({
          id: i.id,
  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-8">
        <Link href={`/invoices/${id}`} className="text-slate-400 hover:text-slate-700"><ArrowLeft className="w-5 h-5" /></Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex gap-3">
          ...
          <Link href={`/invoices/${id}`} className="text-sm font-medium text-slate-500 px-4 py-2 rounded-lg border border-slate-200">Cancel</Link>
        </div>
      </form>
    </div>
  )
